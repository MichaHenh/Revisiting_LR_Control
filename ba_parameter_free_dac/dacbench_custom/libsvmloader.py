...
NAMES = {
    ...
    'sensorless': 'multiclass/Sensorless.scale',
    ...
    'smallNORB': 'multiclass/smallNORB-32x32.xz',
    ...
    'vehicle': 'multiclass/vehicle.scale',
    'vowel': 'multiclass/vowel.scale',
    'wine': 'multiclass/wine.scale',
    'w1a': 'binary/w1a',
...
def _get_X_y(dataset, multilabel, replace=False, verbose=False):
    ...
    if (replace or not y_path.exists()
        ...
        if NAMES[dataset].endswith('.bz2'):
            ...
            source_path.unlink()
        elif NAMES[dataset].endswith('.xz'):
            if verbose:
                print("Decompressing...")
            with open(tmp_path, "wb") as f, lzma.open(source_path) as g:
                for data in g:
                    f.write(data)
            source_path.unlink()
